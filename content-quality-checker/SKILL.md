...
If image(s) are provided:
1. Extract any visible text from the image
2. Cross-check image text vs. caption text for contradictions (e.g., different price, date, product name)
3. Note image-text conflicts as `major` issues under category `image_text_conflict`
4. Do NOT score image aesthetics — only text accuracy
